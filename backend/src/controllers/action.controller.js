...
const getActions = asyncHandler(async (req, res) => {
  ...
  if (!actions.length) {
    return res.json(
      new apiResponse(200, [], 'No actions found for this category')
    )
  }
...
const addUserLog = asyncHandler(async (req, res) => {
  ...
  const actionDef = await userAction.findOne({
  ...
  res
    .status(201)
    .json(new apiResponse(201, log, 'Activity logged successfully'))
})
...
const getUserLogs = asyncHandler(async (req, res) => {
  ...
  res
    .status(200)
    .json(new apiResponse(200, logs, 'User log fetched successfully'))
})
...
const updateUserLog = asyncHandler(async (req, res) => {
  ...
  const actionData = await userAction.findOne({
    actionKey: actionKey.toLowerCase(),
    category: category.toLowerCase(),
  })
  ...
  if (unit !== actionData.baseUnit) {
    const unitConfig = actionData.supportedUnits.find((u) => u.unit === unit)
  ...
  updatedLog.co2 =
    quantityInBaseUnit * actionData.co2PerBaseUnit + (actionData.baseline || 0)

  await updatedLog.save()

  res
    .status(200)
    .json(new apiResponse(200, updatedLog, 'Activity updated successfully'))
})
...
const deleteUserLog = asyncHandler(async (req, res) => {
  ...
  res
    .status(200)
    .json(new apiResponse(200, deletedLog, 'Log deleted successfully'))
})
